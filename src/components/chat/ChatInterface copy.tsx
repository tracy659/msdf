...
import { Send, Paperclip, X, Sparkles, FileText, Zap, Upload, Image, File, FolderOpen } from 'lucide-react';
...
import { DocumentHistorySidebar, UploadedDocumentHistory } from './DocumentHistorySidebar';
...
export function ChatInterface({ caseId, onCaseCreated }: ChatInterfaceProps) {
  ...
  const [conversationState, setConversationState] = useState<'greeting' | 'selecting' | 'details' | 'documents' | 'confirm' | 'complete'>('greeting');
  const [selectedService, setSelectedService] = useState<string | null>(null);
  const [attachedFiles, setAttachedFiles] = useState<File[]>([]);
  const [documentHistory, setDocumentHistory] = useState<UploadedDocumentHistory[]>([]);
  ...
  const handleSendMessage = () => {
    ...
    const userMessage: ChatMessageWithFiles = {
      id: `msg-${Date.now()}`,
      role: 'user',
      content: inputValue || (language === 'ar' ? 'تم إرفاق مستندات' : 'Documents attached'),
      timestamp: new Date().toISOString(),
    ...
    if (conversationState === 'greeting') {
      ...
      const matchedService = services.find(
        (s) => lowerInput.includes(s.nameAr) || lowerInput.includes(s.nameEn.toLowerCase())
      );
      
      if (matchedService) {
        setSelectedService(matchedService.nameAr);
        setConversationState('details');
        simulateAgentResponse(
          agentResponses.serviceSelected[language].replace('{service}', language === 'ar' ? matchedService.nameAr : matchedService.nameEn)
        );
      } else {
        // Default to first matching keyword or ask for clarification
        if (lowerInput.includes('مساعدة') || lowerInput.includes('help') || lowerInput.includes('خدمة') || lowerInput.includes('service')) {
          const defaultService = services[0];
          setSelectedService(defaultService.nameAr);
          setConversationState('details');
          simulateAgentResponse(
            agentResponses.serviceSelected[language].replace('{service}', language === 'ar' ? defaultService.nameAr : defaultService.nameEn)
          );
        } else {
          simulateAgentResponse(agentResponses.fallback[language]);
        }
      }
    } else if (conversationState === 'details') {
      setConversationState('documents');
      simulateAgentResponse(agentResponses.documentsRequest[language]);
    } else if (conversationState === 'documents') {
      if (attachedFiles.length > 0) {
        const price = Math.floor(Math.random() * 1000) + 300;
        setConversationState('confirm');
        simulateAgentResponse(
          agentResponses.documentsReceived[language].replace('{price}', price.toString())
        );
      } else {
        simulateAgentResponse(agentResponses.fallback[language]);
      }
    } else if (conversationState === 'confirm') {
      if (lowerInput.includes('نعم') || lowerInput.includes('yes') || lowerInput.includes('تأكيد') || lowerInput.includes('confirm') || lowerInput.includes('موافق')) {
        // Create case
        const newCase = createCase('social_assistance', selectedService || 'المساعدة الاجتماعية');
        setConversationState('complete');
        simulateAgentResponse(
          agentResponses.caseCreated[language].replace('{caseNumber}', newCase.caseNumber)
        );
        if (onCaseCreated) {
  ...
  return (
    <div className="flex h-full relative">
      {/* Main Chat Area */}
      <div {...getRootProps()} className="flex flex-col flex-1 h-full bg-card rounded-2xl border border-border overflow-hidden shadow-xl relative">
        {/* Drag Overlay */}
        <AnimatePresence>
        ...
        <div className="relative bg-gradient-to-r from-primary via-primary to-primary/90 text-primary-foreground px-5 py-4 overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-10">
            ...
            <div className="absolute bottom-0 right-0 w-24 h-24 bg-accent/30 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '1s' }} />
          </div>
          ...
          <div className="relative flex items-center gap-4">
            ...
            >
              ...
              <motion.div
                ...
                style={{ 
                  background: 'conic-gradient(from 0deg, hsl(344 72% 32%), hsl(344 72% 45%), hsl(43 74% 49%), hsl(344 72% 32%))',
                  filter: 'blur(4px)', 
                  opacity: 0.8 
                }}
              />
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-sm text-primary-foreground/80">
                  {language === 'ar' ? 'متوفر على مدار الساعة للمساعدة' : 'Available 24/7 to assist you'}
                </p>
              </div>
            </div>
